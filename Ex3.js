...
function deleteAnswerNode(tree, answerId) {
  ...
  const index = node.answerList.findIndex(child => child.answerId === answerId);
  const newAnswerList = node.answerList.slice(0, index).concat(node.answerList.slice(index + 1));
  node.answerList = newAnswerList;
  return tree;
}
...
console.log(deleteAnswerNode(decisionTree,6))
